...
in {
    logNode(c, this_indent);
    assert(c.kind.among(CXCursorKind.CXCursor_UnexposedDecl, CXCursorKind.CXCursor_NonTypeTemplateParameter));
}
out (result) {
    logTypeResult(result, this_indent);
}
...
in {
    logNode(c, this_indent);
    logType(type, this_indent);
    assert(type.kind.among(CXTypeKind.CXType_Typedef) || c.kind == CXCursorKind.CXCursor_TypeAliasTemplateDecl);
}
out (result) {
    logTypeResult(result, this_indent);
}
...
in {
    logNode(c, this_indent);
    with (CXCursorKind) {
        assert(c.kind.among(CXCursor_VarDecl, CXCursor_FieldDecl, CXCursor_TemplateTypeParameter, CXCursor_ParmDecl));
    }
}
out (result) {
    logTypeResult(result, this_indent);
...
in {
    logNode(c, this_indent);
    assert(c.kind.among(CXCursorKind.CXCursor_TypeAliasDecl, CXCursorKind.CXCursor_TypeAliasTemplateDecl));
}
out (result) {
    logTypeResult(result, this_indent);
}
...
do {
    ...
    void handleTyperef(ref Nullable!TypeResults rval) {
        ...
        foreach (child; c.children.filterByTypeRef.filter!(a => a.kind == CXCursorKind.CXCursor_TypeRef)
                .takeOne) {
            auto tref = pass4(child, container, indent);
